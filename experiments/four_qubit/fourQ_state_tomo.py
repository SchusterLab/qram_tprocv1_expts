...
def sort_counts_4q(shotsA, shotsB, shotsC, shotsD):
    # data is returned as n0000, n0001, ..., n1111 measured for the 4 qubits
    counts = []
    qcounts_e = [shotsA, shotsB, shotsC, shotsD]
        ...
        for q2_state in (0, 1):
            for q3_state in (0, 1):
                for q4_state in (0, 1):
                    ...
                    and_counts = np.logical_and(and_counts, qcounts[q4_state][3])
...
def make_4q_meas_order():
    ...
    for pauli1 in ('Z', 'X', 'Y'):
        for pauli2 in ('Z', 'X', 'Y'):
            for pauli3 in ('Z', 'X', 'Y'):
                for pauli4 in ('Z', 'X', 'Y'):
                    meas_order.append(pauli1 + pauli2 + pauli3 + pauli4)
...
def make_4q_calib_order():
    ...
    for state0 in ('g', 'e'):
        for state1 in ('g', 'e'):
            for state2 in ('g', 'e'):
                for state3 in ('g', 'e'):
                    calib_order.append(state0 + state1 + state2 + state3)
    return np.array(calib_order)

class AbstractStateTomo4QProgram(QutritAveragerProgram):
    """
    Performs a state_prep_pulse (abstract method) on 4 qubits, then measures in a desired basis.
    ...
    expt = dict(
        ...
        qubits: the qubits to perform the 4 qubit tomography on
        basis: see make_4q_meas_order: measurement bases for the 4 qubits
    ...
    def body(self):
        ...
        # Go to the basis for the tomography measurement (flagged for ZZ correction)
        ...
        self.setup_measure(qubit=qubits[3], basis=self.basis[3], play=True)
    ...
    def collect_counts(self, angle=None, threshold=None):
        ...
        return sort_counts_4q(shots[0], shots[1], shots[2], shots[3])

# ===================================================================== #

class ErrorMitigationStateTomo4QProgram(AbstractStateTomo4QProgram):
    """
    Prep the error mitigation matrix state and then perform 4Q state tomography.
    Experimental Config:
    expt = dict(
        reps: number averages per measurement basis iteration
        qubits: the qubits to perform the 4 qubit tomography on
        state_prep_kwargs.prep_state: gggg, ggge, ggeg, ggee, ..., the state to prepare in before measuring
    )
    """
    def state_prep_pulse(self, qubits, **kwargs):
...
class TestStateTomo4QProgram(AbstractStateTomo4QProgram):
    def state_prep_pulse(self, qubits, **kwargs):
...
class TestStateTomo4QExperiment(Experiment):
    
    def __init__(self, soccfg=None, path='', prefix='TestStateTomography4Q', config_file=None, progress=None):
    ...
    def acquire(self, progress=False):
        ...
        for basis in tqdm(self.meas_order):
            ...
            tomo = TestStateTomo4QProgram(soccfg=self.soccfg, cfg=cfg)
            # print(tomo)
