...
class EgGfPhaseExperiment(Experiment):
    ...
    def acquire(self, progress=False):
        ...
        data = {
            "counts_tomo": np.zeros((self.cfg.expt.expts_phase, len(self.meas_order), len(self.calib_order))),
            "counts_calib": [],
        }
        ...
        if (
            ...
            fid, threshold, angle = hist(data=shot_data, verbose=False, amplitude_mode=full_mux_expt)
            thresholds_q[self.qubit] = threshold[0]
            angles_q[self.qubit] = angle
            ge_avgs_q = get_ge_avgs(
                Igs=Ig[self.qubit],
                Qgs=Qg[self.qubit],
                Ies=Ie[self.qubit],
                Qes=Qe[self.qubit],
        ...
        if swap_qubit == 2:
            ...
            assert False, f"swap_qubit {swap_qubit} is not allowed"
        ...
        assert swap_mod in [
            0,
            2,
        ], f"if you want to measure in the X basis on qubit {qubit}, you need num pi pulses to be even"
        ...
        for i_phase, swap_phase in enumerate(tqdm(phases)):
            ...
            for i_basis, basis in enumerate(self.meas_order):
                ...
                if not test_pi_half:
                    cfg.device.qubit.pulses.pi_EgGf_Q.phase[swap_qubit] = swap_phase
                else:
                    cfg.device.qubit.pulses.pi_EgGf_Q.half_phase[swap_qubit] = swap_phase
    ...
    def analyze(self, data=None, target_X_counts=None):
        """
        target_X_counts: get this from the X basis measurement of the initial state in 1Q tomo, should be n_tomo_corrected counts in [g, e]
        """
        if data is None:
            data = self.data
        ...
        for i_phase in range(self.cfg.expt.expts_phase):
            ...
            counts = tomo_analysis.correct_readout_err([counts], data["counts_calib"])[0]
            # print('adjust', counts)
        ...
        if target_X_counts is None:
            if swap_mod == 0:
                target_X_counts = np.array([1, 0])  # + state measures as g
            else:
                target_X_counts = np.array([0, 1])
        else:
            target_X_counts = np.array(target_X_counts) / sum(target_X_counts)
        self.cfg.expt.final_X_pop = target_X_counts[1]
    ...
    def display(self, data=None):
        ...
        plt.title(
            f'Swap Q{self.cfg.expt.swap_qubit}/Q1, {self.cfg.expt.n_pulses} {"pi/2" if self.cfg.expt.test_pi_half else "pi"} pulses'
        )
        plt.ylim(-0.01, 1.01)
        plt.show()
