...
class RamseyEFExperiment(Experiment):
    ...
    def display(self, data=None, fit=True, **kwargs):
        ...
        if fit:
            p = data['fit_avgi']
            plt.plot(data["xpts"][1:-1], dsfit.decaysin(p, data["xpts"][1:-1]))
            plt.plot(data["xpts"][1:-1], dsfit.expfunc([p[4], p[0], p[5], p[3]], data['xpts'][1:-1]), color='0.2', linestyle='--')
            plt.plot(data["xpts"][1:-1], dsfit.expfunc([p[4], -p[0], p[5], p[3]], data['xpts'][1:-1]), color='0.2', linestyle='--')
            ...
            print(f'T2 Ramsey from fit I: {p[3]}')
        ...
        if fit:
            ...
            print('Suggested new EF frequency from fit Q [MHz]:',
                  f'{self.cfg.device.qubit.f_ef + data["f_ef_adjust_ramsey_avgq"]}')
            print(f'T2 Ramsey from fit Q: {p[3]}')
        plt.tight_layout()
        plt.show()
