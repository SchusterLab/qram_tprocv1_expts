...
class ToFCalibrationProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        if self.dac_ch_type == 'mux4':
            ...
            self.set_pulse_registers(ch=self.dac_ch, style="const", freq=self.freqreg, phase=0, gain=int(self.gain), length=self.pulse_length)
...
class PhotonPumpLoopCalibrationProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.declare_gen(ch=self.dac_ch, nqz=cfg.hw.soc.dacs.res_pump.nyquist, mixer_freq=mixer_freq, mux_freqs=None, mux_gains=None, ro_ch=ro_ch)
        ...
        self.declare_readout(ch=self.adc_ch, length=self.readout_length_adc, freq=self.f_res_pump, gen_ch=self.dac_ch) # gen_ch is used for downconversion on the mux ADC

        assert self.dac_ch_type != 'mux4'
        self.set_pulse_registers(ch=self.dac_ch, style="const", freq=self.f_res_pump_reg, phase=0, gain=self.gain, length=self.pulse_length)
...
class PhotonPumpLoopCalibrationExperiment(Experiment):
    ...
    def display(self, data=None, adc_trig_offset=0, **kwargs):
        ...
        q_ind = self.cfg.expt.qubit
        adc_ch = self.cfg.hw.soc.adcs.readout.ch
        dac_ch = self.cfg.hw.soc.dacs.res_pump.ch

        plt.figure()
        plt.subplot(111, title=f"Time of flight calibration: dac ch {dac_ch} to adc ch {adc_ch}", xlabel="Shot Number", ylabel="Transmission [ADC units]")
