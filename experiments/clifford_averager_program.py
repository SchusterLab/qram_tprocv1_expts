...
def rotate_and_threshold(
    ishots_1q, qshots_1q=None, angle=None, threshold=None, amplitude_mode=False, flip_threshold=False, avg_shots=False
):
    """
    ...
    If flip_threshold is True, does the binning in the opposite direction (less than threshold -> 1)
    ...
    if threshold is not None:
        if not flip_threshold:
            ifinal = np.heaviside(ifinal - threshold, 0)
        else:
            ifinal = np.heaviside(threshold - ifinal, 0)
        qfinal = np.zeros_like(ifinal)
...
def flat_top_pulse(total_length, sigma, sigma_n, npts=500):
    """
    total_length: total length of the pulse
    sigma: sigma of the gaussian ramp up and ramp down
    sigma_n: number of sigmas, so total time ramp up + ramp down is sigma_n * sigma
    """
    times = np.linspace(0, total_length, npts)
    output = np.ones_like(times)
    t_rise = sigma_n * sigma / 2
    output[times < t_rise] = fitter.gaussian(times, a=1, x0=t_rise, sigma=sigma, y0=0)[times < t_rise]
    output[times >= total_length - t_rise] = fitter.gaussian(times, a=1, x0=total_length - t_rise, sigma=sigma, y0=0)[
        times >= total_length - t_rise
    ]
    return times, output
...
class CliffordAveragerProgram(AveragerProgram):
    ...
    def handle_flat_top_pulse(
        ...
        waveformname=None,
        ch=None,
        sigma_ramp_us=0.003,  # us
        sigma_n=4,
        tot_length_us=None,  # us
        freq_MHz=None,
        phase_deg=None,
        gain=None,
        reload=False,
        ...
        """
        Plays a gaussian ramp up (sigma_n/2*sqrt(2) sigmas where true sigma is (sigma/np.sqrt(2)), a constant pulse of length flat_length+4*sigma,
        plus a gaussian ramp down on channel ch.
        By default: sigma=1 ns
        (i.e. sigma, sigma_n follow the slightly broken qick convention)
        """
        if name not in self.pulse_dict.keys() or reload:
            assert None not in [sigma_ramp_us, sigma_n, tot_length_us]
            times_us, I_mhz_vs_us = flat_top_pulse(
                total_length=tot_length_us + sigma_n * sigma_ramp_us,
                sigma=sigma_ramp_us / np.sqrt(2),
                sigma_n=sigma_n * np.sqrt(2),
            )
        else:
            times_us, I_mhz_vs_us = None, None
        ...
        self.handle_IQ_pulse(
            ...
            Q_mhz_vs_us=np.zeros_like(I_mhz_vs_us),
            times_us=times_us,
            freq_MHz=freq_MHz,
            phase_deg=phase_deg,
            gain=gain,
    ...
    def XEgGf_half_pulse(
        ...
        if divide_len:
            ...
            if qDrive == 1:
                f_EgGf_MHz = self.cfg.device.qubit.f_EgGf_half[qDrive]
                gain = self.cfg.device.qubit.pulses.pi_EgGf.half_gain[qDrive]
                sigma_us = self.cfg.device.qubit.pulses.pi_EgGf.half_sigma[qNotDrive]
                if add_virtual_Z:
                    virtual_Z = self.cfg.device.qubit.pulses.pi_EgGf.half_phase[qNotDrive]
            else:
                f_EgGf_MHz = self.cfg.device.qubit.f_EgGf_Q_half[qDrive]
                gain = self.cfg.device.qubit.pulses.pi_EgGf_Q.half_gain[qDrive]
                sigma_us = self.cfg.device.qubit.pulses.pi_EgGf_Q.half_sigma[qDrive]
                if add_virtual_Z:
                    virtual_Z = self.cfg.device.qubit.pulses.pi_EgGf_Q.half_phase[qDrive]
            waveformname += "half"
            name += "half"
        else:
            print("WARNING: using pi pulse params for the swap/2!")
            if qDrive == 1:
                f_EgGf_MHz = self.cfg.device.qubit.f_EgGf[qDrive]
                gain = self.cfg.device.qubit.pulses.pi_EgGf.gain[qDrive]
