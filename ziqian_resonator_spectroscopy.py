...
class ResonatorSpectroscopyProgram(AveragerProgram):
    ...
    def body(self):
        ...
        if self.cfg.expt.pulse_cavity:
            self.setup_and_pulse(ch=self.man_ch, style="const", 
                                 freq=self.freq2reg(self.cfg.expt.cavity_pulse[0], gen_ch=self.man_ch), 
                                 phase=self.cfg.expt.cavity_pulse[3], 
                                 gain=self.cfg.expt.cavity_pulse[1], 
                                 length=self.us2cycles(self.cfg.expt.cavity_pulse[2], gen_ch=self.man_ch))
            self.sync_all() # align channels
        self.measure(
