...
class TomoILC(): 
    ...
    def get_tomo_results(self, IQ_pulse, ZZ_correction=False, evolv_mat=None): 
        ...
        tomo_analysis = TomoAnalysis(nb_qubits=self.nb_qubits,
                                    rfsoc_config=self.rfsoc_config, 
                                    meas_order=self.tomoExpt.meas_order,
                                    calib_order=self.tomoExpt.calib_order)
        
        rho = tomo_analysis.get_rho_from_counts(n_tomo_raw=n_tomo_raw, n_calib=n_calib, pulse_dict=pulse_dict, ZZ_correction=ZZ_correction, evol_mats=self.evolv_mat)
