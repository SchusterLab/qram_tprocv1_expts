...
class Monitoring:
    ...
    def __init__(self,
        ...
        config_path = "S:\Connie\experiments\qram_tprocv1_expts\configs" + config_file
        ...
        self.ZZ_mat = {
            "ge": {
                "value": np.zeros((len(self.qubits), len(self.qubits))),
                    ...
                    if "ge" in self.param_dict[qubit_i].zz.spectro.keys():
                        for key in self.param_dict_default["zz"]["spectro"].keys():
                            if key not in self.param_dict[qubit_i].zz.spectro.ge.keys():
                                self.param_dict[qubit_i].zz.spectro.ge[key] = deepcopy(
                                    self.param_dict_default["zz"]["spectro"][key]
                                )
                    if "ef" in self.param_dict[qubit_i].zz.spectro.keys():
                        for key in self.param_dict_default["zz"]["spectro"].keys():
                            if key not in self.param_dict[qubit_i].zz.spectro.ef.keys():
                                self.param_dict[qubit_i].zz.spectro.ef[key] = deepcopy(
                                    self.param_dict_default["zz"]["spectro"][key]
                                )
                    ...
                    if "ge" in self.param_dict[qubit_i].zz.ramsey.keys():
                        for key in self.param_dict_default["zz"]["ramsey"].keys():
                            if key not in self.param_dict[qubit_i].zz.ramsey.ge.keys():
                                self.param_dict[qubit_i].zz.ramsey.ge[key] = deepcopy(
                                    self.param_dict_default["zz"]["ramsey"][key]
                                )
                    if "ef" in self.param_dict[qubit_i].zz.ramsey.keys():
                        for key in self.param_dict_default["zz"]["ramsey"].keys():
                            if key not in self.param_dict[qubit_i].zz.ramsey.ef.keys():
                                self.param_dict[qubit_i].zz.ramsey.ef[key] = deepcopy(
                                    self.param_dict_default["zz"]["ramsey"][key]
                                )
                    ...
                    if "ge" in self.param_dict[qubit_i].zz.pi.keys():
                        for key in self.param_dict_default["zz"]["pi"].keys():
                            if key not in self.param_dict[qubit_i].zz.pi.ge.keys():
                                self.param_dict[qubit_i].zz.pi.ge[key] = deepcopy(
                                    self.param_dict_default["zz"]["pi"][key]
                                )
                    if "ef" in self.param_dict[qubit_i].zz.pi.keys():
                        for key in self.param_dict_default["zz"]["pi"].keys():
                            if key not in self.param_dict[qubit_i].zz.pi.ef.keys():
                                self.param_dict[qubit_i].zz.pi.ef[key] = deepcopy(
                                    self.param_dict_default["zz"]["pi"][key]
                                )
            ...
            if qubit_i in self.qubit_watched:
                ...
                if "t1" in _param_exp.keys():
                    if "ge" in _param_exp.t1.keys():
                        _expt_t1_ge = Experiment(
                            path=self.expt_path, prefix=f"t1_time_sweep_qubit{qubit_i}", config_file=config_path
                        )
                    ...
                    if "ef" in _param_exp.t1.keys():
                        _expt_t1_ef = Experiment(
                            path=self.expt_path, prefix=f"t1EF_time_sweep_qubit{qubit_i}", config_file=config_path
                        )
                ...
                if "t2" in _param_exp.keys():
                    if "ge" in _param_exp.t2.keys():
                        _expt_t2_ge = Experiment(
                            path=self.expt_path, prefix=f"t2r_time_sweep_qubit{qubit_i}", config_file=config_path
                        )
                        _expt_t2_ge.data = dict(
                            xpts=[],
                            avgi=[],
                            avgq=[],
                            amps=[],
                    ...
                    if "ef" in _param_exp.t2.keys():
                        _expt_t2_ef = Experiment(
                            path=expt_path, prefix=f"t2rEF_time_sweep_qubit{qubit_i}", config_file=config_path
                        )
                        _expt_t2_ef.data = dict(
                            xpts=[],
                            avgi=[],
                            avgq=[],
                            amps=[],
                ...
                if "pi" in _param_exp.keys():
                    if "ge" in _param_exp.pi.keys():
                        _expt_pi_ge = Experiment(path=expt_path, prefix=f"pi_qubit{qubit_i}", config_file=config_path)
                    ...
                    if "ef" in _param_exp.pi.keys():
                        _expt_pi_ef = Experiment(
                            path=expt_path, prefix=f"pi_EF_qubit{qubit_i}", config_file=config_path
                        )
                ...
                if "temp" in _param_exp.keys():
                    _expt_temp = Experiment(path=expt_path, prefix=f"temp_qubit{qubit_i}", config_file=config_path)
                    ...
                    if "ge" in _param_exp.zz.keys():
                        for qubit_ZZ in self.qubit_watched:
                            _expt_zz_ge = Experiment(
                                path=expt_path, prefix=f"zz_qubit{qubit_i}{qubit_ZZ}", config_file=config_path
                            )
                            _expt_zz_ge.data = dict(
                                xpts=[], avgi=[], avgq=[], amps=[], times=[], freq_qb=[], freq_qb_err=[]
                            )
    ...
    def measure_t1(
        self, start_time=None, expt=None, param_exp=None, qubit_test=None, qZZ=None, EF=False, debug=False, save=True
    ):
        ...
        # print('t1 fit', t1_fit)
        # print('t1 fit cov', t1_fit_cov)
        # print('t1 fit err', t1.data['fit_err_amps'])
        # print('t1 fit amps',  t1.data["fit_amps"])
        if save:
            expt.cfg = t1.cfg
            ...
            expt.data["t1_fit"].append(t1_fit)
            expt.data["t1_fit_err"].append(t1_fit_err)
        ...
        if debug:
            ...
            # print the t1 time
            ax.text(0.6, 0.8, "T1: %i +/- %i" % (t1_fit, t1_fit_err), transform=ax.transAxes, fontsize=12)
            ax.legend()

        return t1_fit, t1_fit_err
    ...
    def measure_ramsey(
        ...
        if debug:
            ...
            ax.text(
                0.6,
                0.9,
                "T2: %i +/- %i" % (t2r_fit[3], np.sqrt(t2r_fit_err[3][3])),
    ...
    def measure_spectro(self, qubit_test, qubit_zz=None, debug=False, save=False, EF=False, param_exp=None, expt=None):
        ...
        qspec = meas.PulseProbeSpectroscopyExperiment(
            soccfg=self.rfsoc_config, path=self.expt_path, prefix=prefix, config_file=self.config_path
        )
        ...
        qspec.analyze(fit=True, data=qspec.data)
        qZZspec_fit, qZZspec_fit_err = meas.fitting.get_best_fit(qspec.data)
        
        f0 = qZZspec_fit[2]
        f0_err = np.sqrt(qZZspec_fit_err[2][2])
        ...
        if qubit_zz is not None:
            ...
            ZZ = f0 - freq_bare
            ZZ_err = np.sqrt(f0_err**2 + freq_bare_err**2)
    ...
    def measure_t2(
        ...
        if not accurate_mode:
            ...
            _freq_qb, _freq_qb_err, t2r_fit, t2r_fit_err = self.measure_ramsey(
                start_time=start_time,
                expt=expt,
                param_exp=ramsey_second,
                qubit_test=qubit_i,
                EF=EF,
                debug=debug,
                save=save,
    ...
    def measure_temp(self, qubit_i, debug=False, save=True, start_time=None, expt=None, param_exp=None):
        ...
        # no_eg pulse
        gain_no_eg, contrast_no_eg = self.measure_pi_pulse(
            start_time=start_time,
            expt=expt,
            param_exp=param_exp,
            qubit_test=qubit_i,
        ...
        _param_exp["reps"] = 1000
        _param_exp["rounds"] = 1
        _param_exp = AttrDict(_param_exp)

        # eg pulse
        gain_eg, contrast_eg = self.measure_pi_pulse(
            start_time=start_time,
            expt=expt,
            param_exp=_param_exp,
            qubit_test=qubit_i,
            EF=True,
            debug=debug,
            save=save,
    ...
    def measure_error_amplification(
        self,
        qubit_test,
        qubit_ZZ=None,
        debug=False,
            ...
            for idxq2, qubit_zz in enumerate(self.qubit_watched):
                ...
                if "spectro" in self.param_dict[qubit_test].zz.keys():
                    if "ge" in self.param_dict[qubit_test].zz.spectro.keys():
                        ...
                        # try:
                        ZZ, ZZ_err, freq_qb, freq_qb_err = self.measure_spectro(
                            qubit_test=qubit_test, qubit_zz=qubit_zz, debug=debug, save=save, EF=False
                        )
                    ...
                    if "ef" in self.param_dict[qubit_test].zz.spectro.keys():
                        # try:
                        ZZ, ZZ_err, freq_qb, freq_qb_err = self.measure_spectro(
                            qubit_test=qubit_test, qubit_zz=qubit_zz, debug=debug, save=save, EF=True
                        )
                ...
                if "ramsey" in self.param_dict[qubit_test].zz.keys():
                    if "ge" in self.param_dict[qubit_test].zz.ramsey.keys():
                        # try:
                        freq_qb, freq_err, t2, t2_err = self.measure_ramsey(
                            qubit_test=qubit_test, qubit_ZZ=qubit_zz, EF=False, debug=debug, save=save
                        )
                    ...
                    if "ef" in self.param_dict[qubit_test].zz.ramsey.keys():
                        # try:
                        freq_qb, freq_err, t2, t2_err = self.measure_ramsey(
                            qubit_test=qubit_test, qubit_ZZ=qubit_zz, EF=True, debug=debug, save=save
                        )
        ...
        try:
            ...
            if save:
                for qubit_i in self.qubits:
                    ...
                    data_folder = self.save_path
                    np.savez(data_folder + f"/qubit_{qubit_i}_data", **data_i)
                    # np.savez(f"/qubit_{qubit_i}_data", **data_i)
