...
class TomoAnalysis(): 
    ...
    def pauli(self, i): return [qt.qeye(2), qt.sigmax(), qt.sigmay(), qt.sigmaz()][i]

    def s2i(self, s): # string to pauli matrix index
            return int(np.char.find('IXYZ', s))

    def i2s(self, i): # pauli matrix index to string
            return 'IXYZ'[i]

    def order(self, S): # given a pauli matrix name, get the index at which it (or equivalent data) was measured
    ...
    """
    def counts_from_iqshots(self, tomo_qubits, all_ishots_raw_q_preps, thresholds, ge_avgs=None, nprep=None, ps_adjust=None, ps_qubits=None, n_init_readout=1, angles=None, all_qshots_raw_q_preps=None, verbose=False):
        ...
        if nprep is None: nprep = all_ishots_raw_q_preps.shape[0]

        counts_all = np.zeros((nprep, 2**self.nb_qubits))
        for iprep in range(nprep):
            ps_thresholds = thresholds
            if verbose: print('ps thresholds', ps_thresholds)
            if ps_adjust is not None:
                ps_thresholds = ps_threshold_adjust(ps_thresholds_init=thresholds, adjust=ps_adjust, ge_avgs=ge_avgs, angles=angles)
                if verbose: print('new ps thresholds', ps_thresholds)
    ...
    """
    def n_tomo_calib_from_data(self, data, cfg, tomo_qubits=None, ps_adjust=None, ps_qubits=None, apply_ps=True, verbose=False, ishots_raw=None, qshots_raw=None, calib_ishots_raw=None, calib_qshots_raw=None):
        if apply_ps: assert cfg.expt.readout_cool, "Experiment was not run with post selection readouts!"
        if apply_ps:
            ...
            if ishots_raw is None: ishots_raw = data['ishots_raw']
            if qshots_raw is None: qshots_raw = data['qshots_raw']
            if calib_ishots_raw is None: calib_ishots_raw = data['calib_ishots_raw']
            if calib_qshots_raw is None: calib_qshots_raw = data['calib_qshots_raw']
            if tomo_qubits is None: tomo_qubits = cfg.expt.tomo_qubits
    ...
    def Tij(self, n, S):
        ...
        for icalib, calib in enumerate(self.calib_order):
            for q in range(self.nb_qubits):
                if S[q] > 0 and calib[q] == 'e': signs[icalib] *= -1
    ...
    def rho_from_counts(self, n):
        ...
        for state in state_base4:
            ...
            o = self.order(idx)
            rho += self.Tij(n[o:o+1], state) * qt.tensor(*[self.pauli(int(state[qb])) for qb in range(self.nb_qubits)]) / 2**self.nb_qubits
    ...
    # =========================== Generate test data ====================================== #
    def generate_counts(self, rho, n_tot, psi_basis_flat=None, evol_mats=None, noise=1.0):
        ...
        if psi_basis_flat is not None:
            ...
            for psi in psi_basis_flat:
                ...
                n.append(n_tot * (psi.dag() * rho * psi).tr())
    ...
    def correct_readout_err(self, n, n_conf, verbose=False, method='SLSQP'):
        ...
        if len(n.shape) == 1: n = np.reshape(n, (1, len(n)))
        ...
        for r, n_row in enumerate(n):
            ...
            if method == 'SLSQP':
                cons = [{'type': 'eq', 'fun': constraint_norm},
                        {'type': 'ineq', 'fun': constraint_zero}
                        ]
            elif method == 'COBYLA':
            ...
            # minimize the objective function
            result = minimize(objective, x0_flat, args=(n_row, conf_mat),
                              constraints=cons, method=method, options={'ftol': 1e-20, 'disp': False}, tol=1e-20)
    ...
    def get_evol_mats(self, qubits, qfreqs, alphas, ZZs, pulse_dict, soccfg=None, dt=0.01, debug=False, cutoffs=None):

        assert len(pulse_dict.items()) == len(self.meas_order)
        evol_mats = dict()
        if cutoffs is None: cutoffs = [2]*self.nb_qubits
        ...
        if self.rfsoc_config is None: assert soccfg is not None
        for pulse_name, pulse_cfg in pulse_cfgs_XXX.items():
            ...
            if soccfg is not None: self.rfsoc_config = soccfg
        ...
        for basis, pulse_cfgs in tqdm(pulse_dict.items()):
            ...
            seq = PulseSequence()
            for drive_qubit in range(self.nb_qubits): # drive qubit is the index out of [qubits]
                # For Z basis measurement, do not play a pulse
                if basis[drive_qubit] == 'Z': continue
                ...
                for pulse_name, pulse_cfg in pulse_cfgs.items():
                    ...
                    if debug:
                        print('add pi pulse on qubit', qubits[drive_qubit], 'with freq', freq, 'length', pi2_sigma, 'amp', amp)
                    device.add_precise_pi_pulse(seq, start_state, new_state, amp=amp, pihalf=pihalf, drive_qubit=drive_qubit, wd=wd, phase=phase, type=pulse_type, t_pulse=pi2_sigma, t_offset=0, sigma_n=sigma_n)
            ...
            for init_state in self.calib_order:
                ...
                states_inds = []
                for calib_numeric in self.calib_order_numeric:
                    assert len(calib_numeric) == self.nb_qubits
                    state_ind = 0
                    for q in range(self.nb_qubits):
                        calib_numeric_q = int(calib_numeric[q])
                        if q + 1 == self.nb_qubits: state_ind += calib_numeric_q
                        else: state_ind += calib_numeric_q * np.prod(cutoffs[q+1:])
                    states_inds.append(state_ind)
                if debug: print('truncating to states inds', states_inds)
    ...
    # def get_evol_mats(self, pulse_dict):
    #     assert self.tomo_qubits is not None
    #     # setup the Qswitch 
    #     f_ge = np.array([f + (4000 if f < 1000 else 0) for f in np.reshape(self.config_file.device.qubit.f_ge, (4,4)).diagonal()]) # MHz
    #     f_ef = np.array([f + (4000 if f < 1000 else 0) for f in np.reshape(self.config_file.device.qubit.f_ef, (4,4)).diagonal()]) # Mhz

    #     ZZs_4q = np.zeros((4,4))
    #     reshaped_f_ge = np.reshape(self.config_file.device.qubit.f_ge, (4,4))
    #     for row in range(4):
    #         ZZs_4q[row,:] = reshaped_f_ge[row,:] - reshaped_f_ge[row,row]
    #     f_ge = np.array([f_ge[q] for q in self.tomo_qubits])
    #     f_ef = np.array([f_ef[q] for q in self.tomo_qubits])
    #     alphas = f_ef - f_ge # MHz
    #     ZZs = np.zeros(shape=(self.nb_qubits, self.nb_qubits))
    #     ZZs = ZZs_4q[self.tomo_qubits][:,self.tomo_qubits] # MHz

    #     return self.evol_mats(self.tomo_qubits, f_ge*1e-3, alphas*1e-3, ZZs*1e-3, pulse_dict)
    ...
    def rho_from_counts_ZZcorrection(self, n_tomo, evol_mats):
        ...
        for state in state_base4:
            ...
            for qb in range(self.nb_qubits):
                ...
                else: s_vec.append(int(state[qb]))
            o = self.order(idx)
    ...
    def run_MLE_optimizer(self, n_tomo, rho_guess=None, rho_id=None, method='L-BFGS-B', decimals=10, maxiter=10000000):
        ...
        if rho_guess is None: rho_guess = self.rho_from_counts(n_tomo)
        ...
        def likelihood(t_arr, n_tomo):
            ...
            # return np.log(val)
            return val
        optvals = sp.optimize.minimize(likelihood, self.t_from_rho(rho_guess), args=(n_tomo), method=method, options={'maxiter':maxiter, 'maxfun':maxiter})
        print(f'Convergence: {optvals.success}')
        return np.around(self.rho_from_t(optvals.x), decimals=decimals)

    def run_MLE_ZZ_optimizer(self, n_tomo, evol_mats, rho_guess=None, rho_id=None, method='L-BFGS-B', decimals=10, maxiter=1000000):
        # methods = 'Nelder-Mead' 'Powell' 'CG' 'BFGS' 'Newton-CG' 'L-BFGS-B' 'TNC' 'COBYLA' 'SLSQP' 'dogleg' 'trust-ncg'
        if rho_guess is None: rho_guess = self.rho_from_counts(n_tomo)
        n_tot = np.sum(n_tomo[0,:])
        # psi_basis = get_psi_basis(qubits)
    ...
    """
    def run_MLE(self, n_tomo, analytical=True, ZZ_correction=False, evol_mats=None, rho_guess=None, rho_id=None, method='L-BFGS-B', maxiter=1000000):
        if analytical:
            return self.run_MLE_analytical(n_tomo, ZZ_correction=ZZ_correction, evol_mats=evol_mats) 
        else: 
            if ZZ_correction:
                return self.run_MLE_ZZ_optimizer(n_tomo, evol_mats, rho_guess=rho_guess, rho_id=rho_id, method=method, maxiter=maxiter)
            else:
                return self.run_MLE_optimizer(n_tomo, rho_guess=rho_guess, rho_id=rho_id, method=method, maxiter=maxiter)


    # =========================== PLOTTING FUNCTIONS =========================== #
    def show_mat_2d(self, mat, ax, title, labels, cmax=1, show_cbar=True, show_xticks=True, show_yticks=True, show=True):
        """
        ...
        for ii in range(len(mat)):
            for jj in range(len(mat)):
                plt.text(ii, jj, round(mat[jj, ii], 3), ha="center", va="center", color="w", size=9+6/self.nb_qubits, rotation=45)
        if show_cbar:
    ...
    def show_plot_rho_2d(self, rho_test, rho_id=None, title=None, cmax=None, savetitle=None, size=None):
        ...
        self.show_mat_2d(np.real(rho_test), ax=ax1, title="Re[$\\rho_{MLE}$]", labels=labels, cmax=cmax, show_cbar=False, show_yticks=True, show=False)
        self.show_mat_2d(np.imag(rho_test), ax=ax2, title="Im[$\\rho_{MLE}$]", labels=labels, cmax=cmax, show_cbar=True, show_yticks=False, show=False)
        if rho_id is not None:
            self.show_mat_2d(np.real(rho_id), ax=ax3, title="Re[$\\rho_{Ideal}$]", labels=labels, cmax=cmax, show_cbar=False, show_yticks=True, show=False)
            self.show_mat_2d(np.imag(rho_id), ax=ax4, title="Im[$\\rho_{Ideal}$]", labels=labels, cmax=cmax, show_cbar=True, show_yticks=False, show=False)
    ...
    def show_plot_rho_3d(self, rho_test, rho_id=None, title='', zmin=None, zmax=None, width=0.75, elev=30, azim=-20, savetitle=None):
        if savetitle is None: plt.style.use('default')
        else: plt.style.use('dark_background')
    ...
    # =========================== The Function =========================== #
    def get_rho_from_counts(self, n_tomo_raw, n_calib, method='analytical', correct_readout=True, correct_neg_counts=True, ZZ_correction=False, pulse_dict=None, evol_mats=None, tomo_qubits=None, rho_guess=None, rho_id=None, verbose=False):
        if ZZ_correction: 
            if evol_mats is not None: self.evol_mats = evol_mats
            if self.evol_mats is None:
                assert tomo_qubits is not None
                # First check if the evol_mats have been saved                
                qb_str = ''.join([str(q) for q in tomo_qubits])
                filename = f'evol_mats_{qb_str}.npz'
                evol_mats_path = os.path.join(os.getcwd(), 'evol_mats')
                evol_mats_file_path = os.path.join(evol_mats_path, filename)
                if os.path.exists(evol_mats_file_path):
                    evol_mats = dict()
                ...
                # If not saved, generate the evol_mats
                else:
                    assert pulse_dict is not None 
                    
                    freqs_ge = np.array(self.config_file.device.qubit.f_ge)
                    freqs_ef = np.array(self.config_file.device.qubit.f_ef)
                    freqs_ge *= 1e-3
                    freqs_ef *= 1e-3

                    idxq = [(self.nb_qubits_tot + 1)*q for q in tomo_qubits] 
                    qfreqs = [freqs_ge[idx] for idx in idxq]
                    alphas = [(freqs_ef[idx] - freqs_ge[idx]) for idx in idxq]
                    
                    freq_mat = np.reshape(freqs_ge, (self.nb_qubits_tot, self.nb_qubits_tot))
                    # Reduce the matrix to the qubit we are interested in, i.e take only the corresponding row and column
                    freq_mat = freq_mat[tomo_qubits]
                    freq_mat = freq_mat[:, tomo_qubits]

                    # Define the ZZ matrix by subtracting the frequency of the qubit to each column
                    ZZ_mat = np.zeros((self.nb_qubits, self.nb_qubits))
                    for i in range(self.nb_qubits):
                        ZZ_mat[i, :] = freq_mat[i] - qfreqs[i]

                    evol_mats = self.get_evol_mats(qubits=tomo_qubits, qfreqs=qfreqs, alphas=alphas, ZZs=ZZ_mat, pulse_dict=pulse_dict, debug=False)
                    np.savez(evol_mats_file_path, **evol_mats)

            if self.evol_mats is None:
                self.evol_mats = evol_mats
        ...
        if method == 'MLE':
            ...
            maxiter = None
            rho_id = None
        else: assert False, 'method must be either MLE or analytical'

        n_tomo_corrected = n_tomo_raw
        if correct_readout: n_tomo_corrected = self.correct_readout_err(n_tomo_raw, n_calib)
        ...
        return self.run_MLE(n_tomo_corrected, analytical=analytical, ZZ_correction=ZZ_correction, evol_mats=self.evol_mats, rho_guess=rho_guess, rho_id=rho_id, method=method, maxiter=maxiter)